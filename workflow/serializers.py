...
class ScheduleSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        workflow_codes = filter(
            lambda k: k.startswith(space_code), manager.workflows.keys()
        )
        self.fields["user_code"].choices = [
            workflow_code[len(space_code) + 1 :] for workflow_code in workflow_codes
        ]

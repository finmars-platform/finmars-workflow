...
class WorkflowViewSetFilterTestCase(BaseTestCase):
    def setUp(self):
        ...
        self.workflow3 = Workflow.objects.create(space=self.space, owner=self.user, name="Workflow 3",
            user_code="workflow3", status="pending", payload_data='{"key": "test3"}', workflow_template=workflow_template)

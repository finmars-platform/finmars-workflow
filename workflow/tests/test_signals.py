...
class SignalsTest(TransactionTestCase):
    def setUp(self):
        ...
        task_id = uuid()
        self.workflow = Workflow.objects.create(space=space, owner=User.objects.first(),
                                                status=Workflow.STATUS_INIT)
        self.task = Task.objects.create(workflow=self.workflow, space=space,
                                        celery_task_id=task_id, status=Task.STATUS_INIT)

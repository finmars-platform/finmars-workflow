...
class WorkflowViewSetFilterTestCase(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.workflow_template1 = WorkflowTemplateFactory(
            space=self.space, owner=self.user
        )
    ...
    def test_filter_queryset_date_range(self):
        created_at_after = (
            self.workflow_template1.created_at - timedelta(days=1)
        ).strftime("%Y-%m-%d")
        crated_at_before = (
            self.workflow_template1.created_at + timedelta(days=1)
        ).strftime("%Y-%m-%d")
    ...
    def test_filter_query_user_code(self):
        response = self.client.get(
            self.url, {"user_code": self.workflow_template1.user_code}
        )

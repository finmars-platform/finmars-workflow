...
from workflow_app.utils import (
    ENV_BOOL,
    ENV_INT,
    ENV_STR,
...
FLOWER_URL = ENV_STR("FLOWER_URL", f"/{REALM_CODE}/workflow/flower")
...
WORKFLOW_STORAGE_ROOT = os.path.join(
    BASE_DIR, "workflow_storage"
)  # folder that contains files synced from remote storage
...
DATABASES = {
    "default": {
        ...
        "OPTIONS": {"connect_timeout": 5},  # new timeout setting
    }
}
...
LOGGING = {
    ...
    "formatters": {
        "verbose": {
            ...
            + "] ["
            + REALM_CODE
            + "] [workflow] [%(levelname)s] [%(asctime)s] [%(processName)s] [%(name)s] [%(module)s:%(lineno)d] - %(message)s",
        },
    },
    "handlers": {
        "console": {
...
CELERY_RESULT_BACKEND = (
    f'db+postgresql+psycopg2://{ENV_STR("DB_USER", None)}'
    f':{ENV_STR("DB_PASSWORD", None)}@{ENV_STR("DB_HOST", None)}'
    f':{ENV_INT("DB_PORT", 5432)}/{ENV_STR("DB_NAME", None)}'
)
...
KEYCLOAK_CLIENT_SECRET_KEY = os.environ.get(

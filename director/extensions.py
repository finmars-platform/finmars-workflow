...
from director.storage import get_storage
...
class CeleryWorkflow:
    ...
    def init_app(self):
        ...
        directories, files = storage.listdir(workflow_path)
        ...
        for filename in files:
            ...
            if '.py' in filename:
                ...
                with storage.open(workflow_path + '/' + filename) as f:
                    ...
                    os.makedirs(os.path.dirname(settings.DIRECTOR_LOCAL_DIR + '/tasks/' + filename), exist_ok=True)

                    with open(settings.DIRECTOR_LOCAL_DIR + '/tasks/' + filename, 'wb') as new_file:
    ...
    def import_user_tasks(self):
        ...
        folder = Path(settings.DIRECTOR_LOCAL_DIR).resolve()
